...
public class OrderDAO {
    ...
    public List<OrderEntity> getOrderByStatus(int orderStatus) {
        System.out.println("Consultando pedidos com status: " + orderStatus);
        String ql = "select o from OrderEntity o where o.status = :statusFilter";
        TypedQuery<OrderEntity> q = entityManager.createQuery(ql, OrderEntity.class);
        q.setParameter("statusFilter", orderStatus);
        List<OrderEntity> orders = q.getResultList();
        printOrders(orders);
        return orders;
    }
